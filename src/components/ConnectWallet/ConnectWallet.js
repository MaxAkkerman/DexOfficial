	...
	useEffect(async () => {
		...
		if (!pubKey.status) {
			...
			if (!balanceOF.balanceOf[msigAd] || balanceOF.code) {
				...
				setcurrentStatus(
					"Transfering TONs to your Gas wallet inside the DeX for future operational costs.",
				);
				let tranferToDex = await transfer(
					curExt._extLib.SendTransfer,
			...
			if (
				(dexCLientStatus && dexCLientStatus.code) ||
				!dexCLientStatus.status
			) {
				setcurrentStatus("Connecting wallet");
				dispatch(closeConnecting());
				dispatch(
				...
				if (onShardingStatus.status) {
					...
					let createCLientStatus = await createDEXclient(
						curExt,
						onShardingStatus.data,
					);
