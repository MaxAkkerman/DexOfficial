		...
		if (subscribeData.dst) {
			...
			if (lastTransactioType === "swap") {
				console.log(
					...
					"subscribeData swap",
					subscribeData,
					typeof subscribeData.amountOfTokens,
				);
				if (transactionsList[item])
					transactionsList[item].toValue = Number(
						subscribeData.amountOfTokens / 1e9,
					);
				if (transactionsList.length)
					dispatch(setTransactionsList(transactionsList));
			}
			if (lastTransactioType === "processLiquidity") {
				console.log(
					"item",
					item,
					"subscribeData processLiquidity",
					subscribeData,
				);
				if (transactionsList[item])
					transactionsList[item].lpTokens = Number(
						subscribeData.amountOfTokens / 1e9,
					).toFixed(
						Number(subscribeData.amountOfTokens / 1e9) < 0.0001 ? 6 : 4,
					);
				if (transactionsList.length)
					dispatch(setTransactionsList(transactionsList));
			}
			...
			if (subscribeData.name === "connectCallback") {
				...
				await pairs.map(async (item) => {
					item.exists = await checkClientPairExists(
						pubKey2.dexclient,
						item.pairAddress,
					);
				...
				if (tokenList.length) {
					tokenList.forEach(
						async (item) => await subscribe(item.walletAddress),
					);
			...
			if (swapAsyncIsWaiting) {
				dispatch(
					showPopup({type: "success", link: subscribeData.transactionID}),
				);
				dispatch(
					setSwapFromToken({
						walletAddress: "",
