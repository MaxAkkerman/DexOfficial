...
} = require("../ton-packages/LimitOrderRouter.js");
const {GiverContract} = require("../ton-packages/Giver.js");
const {
	RootTokenContractContract,
} = require("../ton-packages/RootTokenContract.js");
...
const hello = [
	...
	"Hello dev net TON!",
	"Hello main net TON!",
	"Hello rust dev net TON!",
	"Hello fld dev net TON!",
];
...
async function main(client) {
	...
	const rootAddr = JSON.parse(
		fs.readFileSync(pathJson, {encoding: "utf8"}),
	).address;
...
(async () => {
	const client = new TonClient({
		network: {endpoints: [networks[networkSelector]]},
	});
	try {
